...
void DirectCompositionWindow::Initialize(ID3D11Device* _pDevice, HWND _pWindowHandle)
{
    InitializeWindowThread();
    InitializeDirectComposition(_pDevice, _pWindowHandle);
}
...
void DirectCompositionWindow::InitializeDirectComposition(ID3D11Device* _pDevice , HWND _pWindowHandle)
{
  ...
  /*  winrt::com_ptr<ID3D11Device> d3d11Device;
    ...
    _pDevice->QueryInterface(__uuidof(IDXGIDevice), (void**)&dxgiDevice);
    ...
    THROW_IF_FAILED(desktopDevice->CreateTargetForHwnd(_pWindowHandle, TRUE, m_dcompTarget.put()));
