...
GolemSoilderScript::GolemSoilderScript()
	:m_eAttackDir(EnemyDir::DOWN)
	, m_fAccTime(0.f)
	, m_pCollider(nullptr)
{
	SetName(L"GolemSoilderScript");
	SetHp(50);
	SetDamage(5);
	SetChase(FALSE);
}
...
void GolemSoilderScript::Start()
{
	...
	GetOwner()->GetComponent<Animator2D>(COMPONENT_TYPE::ANIMATOR2D)->SetCurAnimation2D(L"FSM_Enemy_GolemSoilder_Walk_Up_Anim");
}

void GolemSoilderScript::Update()
{
	EnemyScript::Update();
}

void GolemSoilderScript::Idle()
{
	if (GetChase())
	{
...
void GolemSoilderScript::Chase()
{
	...
	case EnemyDir::UP:
	{
		if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Walk_Up_Anim")
		{
			_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Walk_Up_Anim", TRUE);
		}
	}
	break;
	case EnemyDir::DOWN:
	{
		if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Walk_Down_Anim")
		{
			_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Walk_Down_Anim", TRUE);
		}
	}
	break;
	case EnemyDir::LEFT:
	{
		if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Walk_Left_Anim")
		{
			_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Walk_Left_Anim", TRUE);
		}
	}
	break;
	case EnemyDir::RIGHT:
	{
		if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Walk_Right_Anim")
		{
			_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Walk_Right_Anim", TRUE);
		}
	}
	break;
	default:
...
void GolemSoilderScript::Attack()
{
	Animator2D* _pAnimator = GetOwner()->GetComponent<Animator2D>(COMPONENT_TYPE::ANIMATOR2D);
	...
	if (m_fAccTime >= 1.5f)
	{
		...
		{
			_vPos.y += 100.f;
			if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Attack_Up_Anim")
			{
				_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Attack_Up_Anim", FALSE);
			}
		...
		{
			_vPos.y -= 100.f;
			if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Attack_Down_Anim")
			{
				_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Attack_Down_Anim", FALSE);
			}
		...
		{
			_vPos.x -= 100.f;
			if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Attack_Left_Anim")
			{
				_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Attack_Left_Anim", FALSE);
			}
		...
		{
			_vPos.x += 100.f;
			if (_pAnimator->GetCurAnimation2D()->GetName() != L"FSM_Enemy_GolemSoilder_Attack_Right_Anim")
			{
				_pAnimator->Play(L"FSM_Enemy_GolemSoilder_Attack_Right_Anim", FALSE);
			}
...
void GolemSoilderScript::OnTriggerEnter(Collider* _other)
{
